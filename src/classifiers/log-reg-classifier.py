...
for j, fold in enumerate(k_fold):
    ...
    log_reg = SGDClassifier(penalty='l2', loss='log')
    ...
    for i in range(0, train_df.shape[0], batch_size):
        ...
        log_reg.partial_fit(features, train_label, classes=unique_classes)
        print("Batch done: ", i)
        break
    dump(log_reg, './Pretrained/log_reg/log_reg_k_' + str(j) + '.joblib')
    all_test_label = []
    all_pred = []
    ...
    plot_confusion_matrix(cnf_matrix, classes=class_names, normalize=True,
                          title='Normalized confusion matrix, k='+ str(j+1))
    plt.savefig("./Plots/log_reg/confusion_"+ "log_reg_" + str(j)+".jpg")
    # plt.show()
    f_time = time.time()
    ...
    break
np.savetxt("./Measures/log_reg/prec_acc_recall.txt", prec_acc_recall)
